<template>
  <div class="block">
    ...
    <input
      v-model="modelValue"
      :type
      :class="{ 'border-green-500': field.$valid, 'border-red-400': field.$error, 'border-orange-300': field.$pending }"
      class="mt-1 block w-full rounded-md border-gray-300 shadow-sm"
      :placeholder
    />
    <span v-if="field.$pending" class="text-orange-300 text-xs"> Checking... </span>
    <ul v-if="field.$tooltips.length" class="text-sm text-gray-500">
